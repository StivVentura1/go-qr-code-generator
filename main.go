...
// Function to resize the logo while maintaining aspect ratio
...
func generateQRCodeWithLogo(data encodeQR, qrPath, logoPath, outputPath string) error {
	...
	// Resize the logo to fit within the QR code (e.g., 20% of QR code size)
	qrBounds := qrImg.Bounds()
	logoSize := qrBounds.Dx() / 5
	resizedLogo := resizeImage(logoImg, logoSize, logoSize)

	// Calculate position to place the logo in the center
	posX := (qrBounds.Dx() - logoSize) / 2
	posY := (qrBounds.Dy() - logoSize) / 2

	// Merge QR code and logo with transparency support
	outputImg := image.NewRGBA(qrBounds)
	draw.Draw(outputImg, qrBounds, qrImg, image.Point{}, draw.Src)
	draw.Draw(outputImg, resizedLogo.Bounds().Add(image.Point{posX, posY}), resizedLogo, image.Point{}, draw.Over)
	...
	fmt.Println("QR code with transparent logo successfully saved to", outputPath)
	return nil
}
...
func main() {
	sampleData := encodeQR{
		...
		MintingID:   "MINT123",
		Name:        "Transparent QR",
		Description: "QR code with a transparent logo",
	}
